...
int hashFunctionMultiplicationMethod(int data, int hashTableSize)
{
    float A = (sqrt(5) - 1) / 2;
    int hashValue = floor(hashTableSize * (data * A - floor(data * A)));
    return hashValue;
}

void addValueMultiplicationMethod(int hashTable[], int data, int hashTableSize, float &loadFactor, int &noOfStoredData, int &collisionCounter)
{
    int hashValue =  hashFunctionMultiplicationMethod(data, hashTableSize);
...
int findValueMultiplicationMethod(int hashTable[], int data, int hashTableSize)
{
    int hashValue =  hashFunctionMultiplicationMethod(data, hashTableSize);
...
void deleteValueMultiplicationMethod(int hashTable[], int data, int hashTableSize, float &loadFactor, int &noOfStoredData)
{
    int hashValue =  hashFunctionMultiplicationMethod(data, hashTableSize);
...
int main()
{
    ...
    if(hashTableSize != -1)
    {
        ...
        for(int i = 0; i < numberOfData; i++)
        {
            ...
            cout << randomNumbers << " ";
            addValueMultiplicationMethod(hashTable, randomNumbers, hashTableSize, loadFactor, storedDataCounter, collisionCounter);
        }
        ...
        while(true)
        {
            ...
            if(choice == 1)
            {
                ...
                cout << "Found at index: " << findValueMultiplicationMethod(hashTable, searchVal, hashTableSize);
            ...
            else if(choice == 2)
            {
                ...
                cin >> delVal;
                deleteValueMultiplicationMethod(hashTable, delVal, hashTableSize, loadFactor, noOfStoredData);
                printHashTable(hashTable, hashTableSize);
			...
			else if(choice == 3)
            {
                ...
                cin >> addVal;
                addValueMultiplicationMethod(hashTable, addVal, hashTableSize, loadFactor, storedDataCounter, collisionCounter);
                printHashTable(hashTable, hashTableSize);
				cout << "Load Factor is:" << loadFactor << endl;
				cout << "Number of collisions: " << collisionCounter << endl;
                cout << endl;
