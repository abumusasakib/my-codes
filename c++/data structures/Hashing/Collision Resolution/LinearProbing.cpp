...
void addValueLinearProbing(int hashTable[], int stateArray[], int data, int hashTableSize, float &loadFactor, int &noOfStoredValues, int &collisionCounter)
{
...
int findValueLinearProbing(int hashTable[], int stateArray[], int data, int hashTableSize)
{
    int hashValue = hashFunctionDivisionMethod(data, hashTableSize);
...
void deleteValueLinearProbing(int hashTable[], int stateArray[], int data, int hashTableSize, float &loadFactor, int &noOfStoredValues)
{
    int hashValue = hashFunctionDivisionMethod(data, hashTableSize);
...
int main()
{
	...
	if(hashTableSize != -1)
	{
		...
		for(int i = 0; i < numberOfData; i++)
		{
			...
			cout << randomNumbers << " ";
			addValueLinearProbing(hashTable, stateArray, randomNumbers, hashTableSize, loadFactor, storedDataCounter, collisionCounter);
		}
		...
		while(true)
        {
            ...
            if(choice == 1)
            {
				...
				cout << "Found at index: " << findValueLinearProbing(hashTable, stateArray, searchVal, hashTableSize);
			...
			else if(choice == 2)
            {
				...
				deleteValueLinearProbing(hashTable, stateArray, delVal, hashTableSize, loadFactor, storedDataCounter);
			...
			else if(choice == 3)
            {
                ...
                cin >> addVal;
                addValueLinearProbing(hashTable, stateArray, addVal, hashTableSize, loadFactor, storedDataCounter, collisionCounter);
                printHashTable(hashTable, hashTableSize);
				cout << "Load Factor is: " << loadFactor << endl;
				cout << "Number of collisions: " << collisionCounter << endl;
                cout << endl;
