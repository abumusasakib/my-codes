...
List<int>? buddy(int start, int limit) {
  for (int n = start; n <= limit; n++) {
    int m = sumDivisors(n) - 1;
    if (m > n && sumDivisors(m) - 1 == n) {
      return [n, m];
    }
  }

  return null; // If no buddy pair is found
}

int sumDivisors(int n) {
  int divisorsSum = 1; // Include 1 as a divisor
  ...
  for (int i = 2; i * i <= n; i++) {
    if (n % i == 0) {
      ...
      divisorsSum += i;
      if (i != n ~/ i) {
        // Add n/i as a divisor (avoid duplication for perfect squares)
        divisorsSum += n ~/ i;
      }
    }
  }
...
void main() {
  print(buddy(10, 50)); // Output: [48, 75]
  print(buddy(48, 50)); // Output: [48, 75]
}